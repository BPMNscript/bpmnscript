...
- **Validator** — checks that a parsed process is structurally sound and reports errors in the editor: undeclared-variable warnings, declared-type mismatches, duplicate attribute keys, missing `class` on a service task, duplicate process/variable/step names, empty branches, and a `goto` that reaches into a `parallel` branch from outside it.
