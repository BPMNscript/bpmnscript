...
Exit codes: `0` success, `1` validation/parse errors, `2` I/O errors. `bpmns build` also prints non-fatal validation warnings (e.g. an undeclared variable reference) to stderr without changing the exit code. `bpmns parse` prints non-fatal import warnings (dropped Operaton extension attributes, lanes) to stderr without changing the exit code, and exits `1` with an actionable message — writing no output file — when the BPMN contains a construct the DSL cannot express.
