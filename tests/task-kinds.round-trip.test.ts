...
describe("idempotence: golden .bpmn -> IR2 -> DSL' -> IR3", () => {
  it("the decompiled DSL' writes each kind back on its own statement", () => {
    ...
    expect(rt.dslPrime).toContain(
      'send NotifyWarehouse(label: "Tell the warehouse", ' +
    ...
    expect(rt.dslPrime).toContain(
      'decide RateRisk(label: "Rate the order risk", decision: "riskRating", ' +
        'binding: latest, mapDecisionResult: singleEntry, ' +
        'resultVariable: "risk")',
    );
    expect(rt.dslPrime).toContain(
      'decide ChooseCarrier(label: "Choose a carrier", ' +
        'class: "com.example.orders.ChooseCarrierDelegate")',
    );
    expect(rt.dslPrime).toContain(
      'decide PriceShipping(label: "Price the shipping", ' +
