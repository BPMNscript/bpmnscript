...
- The reconstruction must be **total**: every valid IR must produce a valid DSL
  string, never throw — the CLI `parse` command must always produce output
...
`goto` fallback". Dominator analysis gives a mechanically checkable criterion for
each structured construct; every edge the catalogue cannot fold degrades to `goto`,
so the decompiler stays total.
