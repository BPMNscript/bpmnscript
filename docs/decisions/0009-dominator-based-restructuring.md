...
Dominator analysis gives a mechanically checkable criterion for each structured construct, and every edge the catalogue cannot fold degrades to `goto`, so the decompiler stays total.
...
- Good, because the algorithm terminates and produces valid DSL for every IR (total over the supported scope)
