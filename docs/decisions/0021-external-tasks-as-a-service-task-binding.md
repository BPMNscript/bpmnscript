...
Both keywords take the same shape and produce the identical BPMN element; only the DSL surface distinguishes them.
...
- Bad, because a diagram viewer or a reader skimming XML sees `operaton:type="external"` as a distinguishing marker that the DSL source no longer surfaces as a distinguishing keyword; the binding is still visible, just as a setting in a `service` step's parens rather than as the statement's own word.
...
- Good, because the keyword itself documents an operational difference, a separately deployed worker, that an attribute value among an element's settings does not surface as prominently.
...
`topic` becomes a fourth legal key in a `service` task's settings, validated by the same exactly-one-of-four check the other three bindings already share.
