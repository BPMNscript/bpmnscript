...
- ADR-0008's structured constructs already hide gateway mechanics behind `if`/`while`/`parallel`.
...
Implicit sequence flow (ADR-0008) and synthesized structural ids (ADR-0010) already follow both rules; this decision makes them binding for what comes next.
