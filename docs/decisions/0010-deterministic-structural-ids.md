...
Chosen option: "Structural coordinate", because it makes the mapping from source
position to synthesized id both deterministic and human-readable, which in turn
makes golden tests stable and makes the `irToDsl`/`astToIr` round-trip
self-documenting.
...
**Validator-side reservation:** gateway ids deliberately bypass the
`taken`/`resolveCollision` guard — the position-path scheme is injective by
construction, so no runtime collision is possible. However, an author-chosen
statement name that matches a synthesised-id pattern would still produce
duplicate-id IR if allowed through. To close this gap, the
`checkReservedNames` validator in
`packages/language/src/bpmn-script-validator.ts` rejects any statement name
matching `Gateway_*_(split|join|fork|loop)`, `StartEvent_*`, `EndEvent_*`, or
the two-segment `Flow_*_*` form at parse/validation time. The `Flow_` pattern
is deliberately tightened to two segments (`/^Flow_.+_.+$/`) because
synthesised flow ids occupy only `SequenceFlow.id` (never node names) — a
single-segment name like `Flow_Control` cannot collide and is therefore
accepted.
